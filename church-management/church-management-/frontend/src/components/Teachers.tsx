...
import { Search, X, AlertCircle, Users, Mail, Phone, Pencil } from 'lucide-react';
...
function EditTeacherDialog({
  ...
  const [form, setForm] = useState({ full_name: teacher.full_name, phone: teacher.phone ?? '', role: teacher.role });
        ...
        <DialogContent className="max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button type="submit" disabled={loading}>{loading ? 'Enregistrement…' : 'Sauvegarder'}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
...
function TeacherCard({ teacher, onUpdated }: { teacher: Teacher; onUpdated: (t: Teacher) => void }) {
  const gradient = getAvatarGradient(teacher.id);
  const teamColorBg = teacher.team?.color ? (TEAM_COLOR_MAP[teacher.team.color] ?? 'bg-gray-400') : null;
    ...
    <Card className="group hover:shadow-md transition-shadow">
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          {/* Avatar */}
          <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center shrink-0 text-white font-bold text-sm`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              <Badge variant="outline" className={`text-xs ${ROLE_COLORS[teacher.role] ?? ''}`}>
                {teacher.role}
              </Badge>
              {teacher.team && (
                <div className="flex items-center gap-1">
                  <div className={`w-2 h-2 rounded-full ${teamColorBg}`} />
                  <span className="text-xs text-muted-foreground">{teacher.team.name}</span>
                </div>
              )}
              {!teacher.team && (
                <span className="text-xs text-muted-foreground italic">Sans équipe</span>
              )}
            </div>
...
export default function Teachers({ profile: _profile }: TeachersProps) {
  ...
  const roleCount = (role: string) => teachers.filter((t) => t.role === role).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {teachers.length} membre{teachers.length !== 1 ? 's' : ''} enregistré{teachers.length !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-sm">
        ...
        <Input placeholder="Rechercher un professeur…" className="pl-9"
          ...
          <CardContent className="text-center py-16 text-muted-foreground">
            ...
            {(search || filterRole !== 'all') && (
              <button onClick={() => { setSearch(''); setFilterRole('all'); }}
                className="text-sm underline mt-1">Réinitialiser les filtres</button>
            )}
          </CardContent>
        </Card>
      ) : (
